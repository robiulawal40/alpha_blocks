...
function Sidebar(props) {
	...
	return (
		<div className="alpb_icons_library_sidebar">
			<SearchControl
				value={search}
				onChange={(newValue) => {
					setSearch(newValue);
				}}
				placeholder={__("Search Icon", "alpb")}
			/>

			{!isEmpty(preparedIconPacks) && (
				<MenuGroup className="alpb_icons_library_sidebar_item_group">
					{preparedIconPacks.map((iconPack, index) => {
						return (
							<PanelBody key={iconPack?.title} title={iconPack?.title} initialOpen={index === 0}>
								{iconPack?.categories.map((category) => {
									return (
										<MenuItem
											key={category?.name}
											className="alpb_icons_library_sidebar_item"
